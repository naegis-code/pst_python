...
query = text(f"""
            select stcode ,
                ...
                "location" ,
                skcode ,
                sum(case when rpname = '{table_type.upper()}1' then cntqnt else 0 end) as first,
                sum(case when rpname = '{table_type.upper()}2' then cntqnt else 0 end) as final
            from {table_bu}
             where (cntdate BETWEEN '{date_start}' AND '{date_end}')
                and "location" not like 'DOC%'
             group by stcode,cntdate ,skcode,"location"
