...
def standardize_runvar(bu,rpname,usecol):
    ...
    for index, file in enumerate(files, 1):
        ...
        try:
            ...
            # Assuming var files are for Credit
            df['skutype'] = 'Credit'  

            df['rpname'], df['bu'], df['stcode'], df['cntdate'] = cols1, cols2, cols3, cols4

            # Compare first 5 characters of countname with cols3
            if (df['store'] != cols3).any():
                print(f"⚠️ {file}: Some 'countname' values do not match the filename : {file} Content {df['countname'].iloc[0][:5]} != {cols3}.")
                return
            ...
            if df.empty:
                print(f"🚫 {rpname.upper()} No plan found for {file} ({index}/{files})")
                return
            ...
            query_delete = text(f"""
                ...
                AND countname = '{df['countname'].iloc[0]}'
                AND skutype = '{df['skutype'].iloc[0]}'
            """)
            ...
            if not df_na.empty:
                ...
                with connect_db_result.begin() as conn:
                    ...
                    print(f"🗑️ Deleted old data for {df['rpname'].iloc[0]}, {df['countname'].iloc[0]}, {df['skutype'].iloc[0]}")
...
def standardize_runstk(bu,rpname,usecol):
    ...
    for index, file in enumerate(files, 1):
        ...
        try:
            ...
            if (df['store'] != cols3).any():
                print(f"⚠️ {file}: Some 'stcode' values do not match the filename : {file} Content {df['store'].iloc[0]} != {cols3}.")
                return
            ...
            if not df_na.empty:
                ...
                with connect_db_result.begin() as conn:
                    conn.execute(query_delete)
                    print(f"🗑️ Deleted old data for {df['rpname'].iloc[0]}, {df['countname'].iloc[0]}")
                df_clean = df.drop(columns=['recheck', 'stcode', 'bu'], errors='ignore')
                df_clean.to_sql(table_name, connect_db_result, if_exists='append', index=False)
                shutil.move(str(dst_path / file), str(cpt_path / file))
                print(f"♻️ Reinserted {file} ({len(df_clean)} rows) ({index}/{len(files)})")
