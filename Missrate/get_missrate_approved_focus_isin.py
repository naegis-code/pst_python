...
cond_df['cntdate'] = pd.to_datetime(cond_df['cntdate'], format='%Y%m%d', errors='coerce')

# ----------------------------
try:
    for index, row in cond_df.iterrows():
        stcode = row['stcode']
        cntdate_threshold = row['cntdate']
        ...
        df = df[(df['stcode'] == stcode) & (df['cntdate'] == cntdate_threshold)]
    ...
    print(f"{stcode}_{cntdate_threshold.strftime('%Y%m%d')}: An error occurred: {e}")
