...
bu = 'ssp'
sheet_stk1 = 'detailsku(first)'
sheet_var1 = 'type_f_full'
column_stk1 = 'A:X'
column_var1 = 'A:V'
copy_f_path = filepath / 'Shared' / 'Performance' / 'report' / 'report1' / bu.upper()

f_path = pathlib.Path.home() / 'Documents' / 'soh' / 'report1' / bu.upper() / 'STORE'
t_path = pathlib.Path.home() / 'Documents' / 'soh' / 'Completed'


# ✅ Move with overwrite (skip files containing "_W" after BU)
for item in copy_f_path.glob("*"):
    if item.is_file():
        # ✅ ข้ามไฟล์ที่มี "_W" หลัง BU
        if f"_{bu.upper()}_W" in item.name:
            print(f"Skipped file (contains W): {item.name}")
            continue
...
for i, file in enumerate(excel_files, start=1):
    ...
    try:
        ...
        if sheet_stk1 in xls.sheet_names:
            ...
            df['qty_count'] = pd.to_numeric(df['qty_count'], errors='coerce').fillna(0).round(3)
            df['retail'] = pd.to_numeric(df['retail'], errors='coerce').fillna(0).round(3)
            df['cost'] = pd.to_numeric(df['cost'], errors='coerce').fillna(0).round(3)

            df['phycnt_rtl'] = df['qty_count']*df['retail']
            df['phycnt_cst'] = df['qty_count']*df['cost']
