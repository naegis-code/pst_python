...
path_report = filepath / 'Apps' / 'Stocktake' / 'ssp_stk_report.csv'
path_report_dept = filepath / 'Apps' / 'Stocktake' / 'ssp_stk_report_dept.csv'
...
q_report = f"""
                select store as stcode ,cntdate ,rpname ,skutype ,
                    count(*) as sku_count,
                    sum(
                        case when qty_var = 0 then 1 else 0 end) as sku_eq,
                    sum(
                        case when qty_var > 0 then 1 else 0 end) as sku_gain,
                    sum(
                        case when qty_var < 0 then 1 else 0 end) as sku_loss,
                    sum(soh) as qnt_soh,
                    sum(qty_count) as qnt_physical,
                    sum(
                        case when qty_var > 0 then qty_var else 0 end) as qnt_gain,
                    sum(
                        case when qty_var < 0 then qty_var else 0 end) as qnt_loss,
                    sum(qty_var) as qnt_variance,
                    sum(phycnt_rtl-extrtl_var) as retail_soh,
                    sum(phycnt_cst-extcst_var) as cost_soh,
                    sum(phycnt_rtl) as retail_physical,
                    sum(phycnt_cst) as cost_physical,
                    sum(
                        case when extrtl_var > 0 then extrtl_var else 0 end) as retail_gain,
                    sum(
                        case when extcst_var > 0 then extcst_var else 0 end) as cost_gain,
                    sum(
                        case when extrtl_var < 0 then extrtl_var else 0 end) as retail_loss,
                    sum(
                        case when extcst_var < 0 then extcst_var else 0 end) as cost_loss,
                    sum(extrtl_var) as retail_net,
                    sum(extcst_var) as cost_net
                from ssp_stk_this_year ssty 
                where cntdate between '{sdate}' and '{edate}'
                group by store ,cntdate ,rpname ,skutype
                """
